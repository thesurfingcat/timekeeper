class SendUcasChangecourse < Struct.new(:personalId, :applicationSchemeCode, :choiceNumber, :courseCode, :campus, :pointOfEntry, :entryYear, :entryMonth, :choice_id, :currentUser)
  ...
  def perform
    ...
    begin
      ...
      result = driver.setCourseCorrectionTransUcas(institutionCode, userId, password, '', personalId, applicationSchemeCode, choiceNumber, courseCode, campus, pointOfEntry, entryYear, entryMonth)
    
      if result.uCASResponse.status == "SUCCESS"
        Ucasdecisionlog.create(:decisionDate => DateTime.now, :webmethod => 'setCourseCorrectionTransUcas', :outcome => 'Success', :userId => currentUser, :personalId => personalId, :applicationSchemeCode => applicationSchemeCode, :choiceNumber => choiceNumber, :courseCode => courseCode, :campus => campus, :pointOfEntry => pointOfEntry, :yearOfEntry => entryYear, :monthEntry => entryMonth)

class ChoiceReport < ApplicationReport
  def initialize(app,choice) 
    ...
    @letter_type = 'UC'
    @name = @app.title_id.to_s + " " + @app.given_names.to_s + " " + @app.surname.to_s
    ...
    if @choice.int_deposit_amount_req == nil or @choice.int_deposit_amount_req == " " or @choice.int_deposit_amount_req == 0
      ...
      @govsponsor = " and also pay a deposit of £" + @choice.int_deposit_amount_req.to_s + " <bold>within 21 days.</bold> Once you have paid this deposit we will issue you with a visa support letter. You can use this offer letter and your visa support letter to apply for your visa."  
    end
  ...
  def render_body 
    ...
    pdf.font_size 10   
    pdf.move_down(50)
    pdf.text @app.title_id.to_s + " " + @app.given_names.to_s + " " + @app.surname.to_s
    ...
    pdf.text @app.home_countrycode 
    pdf.move_down(10)
    pdf.text Time.now.strftime("#{Time.now.day.ordinalize} %B %Y")
    pdf.move_down(10) 
    pdf.text "Dear " + @app.given_names.to_s + ","      
    pdf.move_down(10) 
    ...
    if @choice.decision == 'C'
      pdf.text "This offer is subject to you:"
    ...
    pdf.move_down(10) 
    pdf.text"You should expect to pay approximately £600 per month towards living expenses (which will include your accommodation). Information about accommodation costing and an accommodation application form are available from <a href='http://www.glam.ac.uk/accommodation/'>www.glam.ac.uk/accommodation</a>."
    pdf.move_down(10)
    ...
    if (@app.agent_name != nil)
    pdf.text "If you have any further queries regarding your offer please contact our local representative " + Agentcode.find(@app.agent_name).description + " or us in the Enquiries and Admissions Unit."   
    else
